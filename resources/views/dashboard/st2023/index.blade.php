            ...
            getSipmenData(){
                ...
                for (let i=0;i< self.list_kab.length;++i) {
                    ...
                    $.ajax({
                        ...
                        let total_rusak = data.data
                                        .filter(item => item.kd_dokumen=="ST2023.L2-UTP")
                                        .reduce((sum, x) => {
                                                if(typeof x === 'object'){
                                                    if('jml_terima' in x) sum += (parseInt(x.jml_rusak) + parseInt(x.jml_kosong))
                                                }
                                                return sum;
                                            }, 0)
                        const element_rusak = document.getElementById("sipmen_rusak" + currKab);
                        element_rusak.innerHTML = total_rusak;
                    }).fail(function (msg) {

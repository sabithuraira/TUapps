    ...
    <table class="table m-b-0">
        ...
        @else
            <thead>
                ...
                <th>{{ $datas[0]->attributes()['jadwal_dinas'] }}</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$data['jadwal_dinas']}}</td>
                    <td>{{$data['batas_penilaian']}}</td>
                    <td>{{$data['pelaksana']}}</td>
                    <td>{{$data['bukti_fisik']}}</td>
                    
                    <td class="text-center"><a href="{{action('JadwalDinasController@edit', $data['id'])}}"><i class="icon-pencil text-info"></i></a></td>
                    <td class="text-center">
                    <form action="{{action('JadwalDinasController@destroy', $data['id'])}}" method="post">
                        @csrf

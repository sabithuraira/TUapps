<div id="app_vue">
    <div class="form-group">
        ...
        <div class="input-group date" data-date-autoclose="true" data-provide="datepicker">
            <input type="text" class="form-control {{ $errors->first('tanggal') ? ' parsley-error' : '' }}"
                id="tanggal" name="tanggal" value="{{ old('tanggal', date('m/d/Y', strtotime($model->tanggal))) }}"
                autocomplete="off">
    ...
    <div class="form-group">
        ...
        <select class="form-control {{ $errors->first('waktu') ? ' parsley-error' : '' }}" name="waktu">
            @foreach ($item_waktu as $iwaktu)
                <option value="{{ $iwaktu['id'] }}" @if ($iwaktu['id'] == old('waktu', $model->waktu)) selected="selected" @endif>
                    {{ $iwaktu['waktu'] }}
                </option>
            @endforeach
        </select>
        @foreach ($errors->get('waktu') as $msg)
            <p class="text-danger">{{ $msg }}</p>
        @endforeach
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="isi" class="summernote form-control {{ $errors->first('isi') ? ' parsley-error' : '' }}"
            name="isi" value="{{ old('isi', $model->isi) }}" data-provide="markdown" rows="10"></textarea>

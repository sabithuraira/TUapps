            ...
            <tr v-for="(data, index) in datas" :key="data.id">
                <td class="px-2">
                    <a href="#" role="button" v-on:click="updateIzinKeluar" data-toggle="modal" 
                            ...
                            :data-pegawai_nip="data.pegawai_nip" 
                ...
                <td class="text-center pt-2">
                    @{{ data.name }}
                    <p class="text-muted">@{{ data.pegawai_nip }}</p>
                </td>
...
@include('izin_keluar.modal_form_eks')
...
var vm = new Vue({  
    ...
    data:  {
        ...
        form_user: {!! json_encode($form_user) !!},
        pathname : window.location.pathname,
        form: {
            id: 0, pegawai_nip: '', keterangan: '',
            tanggal: '', start: '', end: '', pegawai_nip: '', jenis_keperluan: 2
        }
    },
    watch: {
        month: function (val) {
    ...
    methods: {
        ...
        updateIzinKeluar: function (event) {
            ...
            if (event) {
                ...
                $('#form_pegawai_nip').val(event.currentTarget.getAttribute('data-pegawai_nip')).trigger('change');
            ...
            if(self.form.tanggal.length==0 ||   
                ...
                $.ajax({
                    ...
                    data:{
                        ...
                        pegawai_nip: self.form.pegawai_nip, 
                    },
                }).done(function (data) {
        ...
        setDatas: function(){
            ...
            $.ajax({
                url : (self.pathname).replace("/index_eks", "") + "/data_izin_keluar",
                method : 'post',
                dataType: 'json',
                data:{
                    start: self.month, 
...
$(document).ready(function() {
    ...
    $('#form_pegawai_nip').on("select2-selecting", function(e) {
        vm.form.pegawai_nip = e.choice.id
    });
});
